    ...
    useEffect(() => {
        const initMediaPipe = async () => {
            try {
                ...
                const vision = await FilesetResolver.forVisionTasks(new URL('wasm/', pageBase).href);

                visionRef.current.poseLandmarker = await PoseLandmarker.createFromOptions(vision, {
                    baseOptions: {
                        modelAssetPath: new URL('models/pose_landmarker_lite.task', pageBase).href,
                        delegate: "GPU"
                    },
                    runningMode: "VIDEO",
                    numPoses: 1
                ...
                console.error(err);
                setStatus("Failed to load local pose model.");
            }
        };
        ...
        return () => {
            ...
            visionRef.current.poseLandmarker?.close?.();
            if (SoundSynth.ctx && SoundSynth.ctx.state !== 'closed') {
                SoundSynth.ctx.close().catch(() => undefined);
            }
        };
    }, []);
        ...
        <div className="flex flex-col items-center gap-5 w-full max-w-4xl relative">
            ...
            <div className="relative">
                ...
                <button
                    ...
                    className={`group absolute top-[4%] left-[2.5%] z-20 w-[3%] h-auto text-[#535353] hover:text-[#333] transition-colors focus:outline-none focus:ring-2 focus:ring-[#F59E0B] rounded-sm aspect-square`}
                    ...
                    <div className="absolute top-0 left-0 w-full h-full bg-white/80 flex flex-col items-center justify-center z-10 rounded-lg">
                        {isLoading ? (
                            <>
                                ...
                                <p className="font-press-start text-xs text-[#535353]">Loading Local Pose Model...</p>
                            </>
                        ) : (
                            <div className="flex flex-col items-center gap-4">
                                {!cameraReady ? (
                                    <button
                                        onClick={enableCam}
                                        className={`px-5 py-3 bg-transparent border-2 border-[#535353] text-[#535353] font-press-start text-base cursor-pointer hover:bg-[#535353] hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-[#F59E0B] focus:ring-offset-2`}
                                    >
                                        ENABLE CAMERA
                                    </button>
                                ) : (
                                    <>
                                        <button
                                            onClick={manualStart}
                                            className={`px-5 py-3 bg-[#535353] border-2 border-[#535353] text-white font-press-start text-base cursor-pointer hover:bg-[#F59E0B] hover:border-[#F59E0B] transition-colors focus:outline-none focus:ring-2 focus:ring-[#F59E0B] focus:ring-offset-2`}
                                        >
                                            START GAME
                                        </button>
                ...
                {(hasStarted && !gameRunning) && (
                    <div className="absolute top-0 left-0 w-full h-full bg-black/50 flex flex-col items-center justify-center z-10 rounded-lg text-center p-2 md:p-4">
                        ...
                        <div className="text-white/90 text-[8px] font-press-start bg-black/40 p-1 md:p-2 rounded">
                            Pose detection: MediaPipe (local on-device model)
                        </div>
                    </div>
                )}
            </div>
            ...
            <p className="text-[10px] text-[#777] text-center font-press-start leading-relaxed max-w-2xl px-3">
                Camera processing stays in this browser. This app does not record or upload camera video or pose data.
            </p>
            ...
            <div className="mt-2 text-xs text-[#888] text-center font-press-start flex gap-4">
                <label className="cursor-pointer flex items-center justify-center hover:text-[#535353] transition-colors">
                    <input
                        ...
                        className={`mr-2 w-4 h-4 accent-[#F59E0B] focus:outline-none focus:ring-2 focus:ring-[#F59E0B] focus:ring-offset-2 focus:ring-offset-[#f7f7f7] rounded cursor-pointer`}
                    />
